...
class DownloadPanel(Screen):
    ...
    def on_pre_enter(self):
        ...
        self.video_path = "/".join(self.videoinfo.path.split("/")[:-1]) + "/temp_file1"
        self.audio_path = "/".join(self.videoinfo.path.split("/")[:-1]) + "/temp_file2"
    ...
    def merge_videos(self):
        ...
        self.thread.join()
        try:
            subprocess.run(["ffmpeg", "-y", "-i", self.video_path, "-i", self.audio_path, "-c", "copy", "-strict", "-2", self.videoinfo.path], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            success = True
